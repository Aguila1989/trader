  ...
  lessons: [
    {
      ...
      paragraphs: [
        ...
        "You choose the model, not just the provider: in Settings > Account > AI API Key there is an optional Model field. Leave it empty and Atrium uses a sensible default for your provider; fill in a specific model id (for example a cheaper or a more capable variant) and every AI request runs on exactly that model, on your own bill. The Test connection button checks the model id together with your key, so a typo fails before you save rather than at your first trade proposal.",
      ],
      example:
